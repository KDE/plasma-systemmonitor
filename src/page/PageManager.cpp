...
std::optional<fs::path> PageManagerPrivate::renameOldPage(PageController *controller, int version, const QString &newName)
{
    if (controller->m_version != version || controller->m_writeableState != PageController::WriteableState::Writeable) {
        return std::nullopt;
    }
...
QString PageManagerPrivate::determineFileName(const QString &initialFileName)
{
    ...
    if (baseName.endsWith(PageExtension)) {
        baseName.replace(PageExtension, ""_L1);
    }
...
void PageManager::load()
{
    ...
    for (const auto &info : d->s_replacePages) {
        if (!pages.contains(info.oldName)) {
            continue;
        }

        auto controller = pages.value(info.oldName);
        if (auto newPath = d->renameOldPage(controller, info.version, info.newName); newPath.has_value()) {
            pages.remove(info.oldName, controller);
            pages.insert(info.newName, controller);
            controller->m_path = newPath.value();

            pages.value(info.oldName)->m_replacedOutdated = true;
        }
    }

    auto keys = pages.keys();
    auto removed = std::ranges::unique(keys);
    for (auto itr = keys.begin(); itr != removed.begin(); ++itr) {
        auto controller = pages.value(*itr);
        ...
        auto replaceItr = std::ranges::find_if(PageManagerPrivate::s_replacePages, [controller](const auto &entry) {
            return entry.newName == controller->fileName();
        });
        if (replaceItr != PageManagerPrivate::s_replacePages.end() && controller->m_writeableState == PageController::WriteableState::NotWriteable) {
            continue;
        }
        ...
        if (controller->writeableState() != PageController::WriteableState::NotWriteable) {
            auto fileCount = pages.count(*itr);
...
PageController *PageManager::importPage(const QUrl &file)
{
    ...
    controller->load();
    controller->m_version = controller->data()->property("version").toInt();
